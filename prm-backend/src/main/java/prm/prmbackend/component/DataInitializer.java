...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        boolean forceSeed = "true".equalsIgnoreCase(System.getenv("FORCE_SEED"));
        boolean isDevProfile = Arrays.asList(environment.getActiveProfiles()).contains("dev");
        boolean mangaEmpty = mangaRepository.count() == 0;

        if (forceSeed) {
            log.info("FORCE_SEED=true — running full seed");
            runFullSeed();
        } else if (isDevProfile && mangaEmpty) {
            log.info("Profile=dev and manga collection is empty — running full seed");
            runFullSeed();
        } else {
            log.info("Skipping manga/chapter seed (not dev profile or data already exists). " +
    ...
    private void runFullSeed() {
        ...
        seedLicensedMangas(genres, tags, creators);
        seedDemoMangas(genres, tags, creators);
    }
