...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _buildFeaturedManga(MangaProvider provider) {
    ...
    if (provider.mangas.isEmpty && provider.recommendedMangas.isEmpty) {
    ...
    final source = provider.recommendedMangas.isNotEmpty
        ? provider.recommendedMangas
        : provider.mangas;
    final featuredManga = source.first;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [
            BoxShadow(color: AppColors.primaryContainer, offset: Offset(6, 6)),
          ],
        ),
        child: Stack(
          fit: StackFit.expand,
  ...
  Widget _buildMangaGrid(MangaProvider provider) {
    ...
    // "Recently updated" uses the dedicated latest endpoint when available.
    final items = provider.latestMangas.isNotEmpty
        ? provider.latestMangas
        : provider.mangas;
